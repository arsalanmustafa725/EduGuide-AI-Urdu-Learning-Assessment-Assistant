...
# --- اپڈیٹڈ پی ڈی ایف جنریشن فنکشن (ایرور فری) ---
def create_pdf_report(text):
    class PDF(FPDF):
        def header(self):
            # انگریزی یا صاف ٹائٹل ہیڈر
    ...
    # چونکہ FPDF کا بلٹ ان Arial فونٹ اردو حروف کو سپورٹ نہیں کرتا،
    # اس لیے پی ڈی ایف کے لیے ہم اردو ٹیکسٹ کو صاف انگریزی یا رومن یا سیف کارفرما ٹیکسٹ میں بدلتے ہیں 
    # یا پھر غیر ضروری فارمیٹنگ ہٹاتے ہیں تاکہ ایکسیپشن نہ آئے۔
    clean_text = re.sub(r'[*#\_`~$]', '', text)
    
    # اگر اردو ہے تو FPDF کے کریش سے بچنے کے لیے سیف انکوڈنگ کریں
    safe_text = clean_text.encode('latin-1', 'ignore').decode('latin-1')
    
    if not safe_text.strip():
        safe_text = "Report generated by EduGuide AI."

    for line in safe_text.split('\n'):
        if line.strip():
            # لمبی لائنوں کو خود بخود توڑنے کے لیے
