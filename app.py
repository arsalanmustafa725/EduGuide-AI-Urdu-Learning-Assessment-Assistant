...
# --- 🎯 محفوظ اور 100% ورکنگ چینی و غیر ملکی حروف ہٹانے کا فنکشن ---
def remove_foreign_characters(text):
    if not text:
        return ""
    # چینی (CJK Unified Ideographs) اور ہندی/دیوناگری کی سہی رینج
    # یہ فنکشن صرف غیر ملکی حروف کو مٹائے گا، اردو، انگریزی اور نمبرز کو بالکل نقصان نہیں پہنچائے گا
    pattern = r'[\u4e00-\u9fff\u3400-\u4dbf\u0900-\u097f]+'
    ...
    if not clean_input_text:
        ...
        try:
            with st.spinner('EduGuide AI Urdu جواب تیار کر رہا ہے...'):
                ...
                # 🔥 سکرین پر دکھانے سے پہلے چینی حروف کو ہٹانا
